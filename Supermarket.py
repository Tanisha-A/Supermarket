...
class Seller:
    ...
    def add_product(self):
        ...
        product = product.casefold() #returns string where all characters are in lower case
        mycursor.execute("SELECT * FROM seller_items WHERE name = '{}'".format(product))
        myresult = bool(mycursor.fetchall()) #fetch all rows from database table using cursor's fetchall()
        if myresult == True:
            ans = input("This item is already present in the inventory do you want to add quantity to this item? : ")
            ans = ans.capitalize() #returns a copy of the original string and converts the first character of the string to a capital (uppercase) letter while making all other characters in the string lowercase letters.
            if ans == 'No':
                pass
            elif ans == 'Yes':
